...
export function analyzeJournal(rawText) {
  let line;
  let rawHash;
  try {
    requireValue(typeof rawText === 'string', 'RAW_TEXT_REQUIRED');
    requireValue(Buffer.byteLength(rawText, 'utf8') <= MAX_BYTES, 'JOURNAL_BYTE_LIMIT');
    rawHash = digest(rawText);
    ...
    for (const row of rows.slice(1, -1)) {
      ...
      requireValue(typeof data.collector_output === 'string'
        && Buffer.byteLength(data.collector_output, 'utf8') <= MAX_OUTPUT_BYTES,
...
function cli(args) {
  if (args.length !== 1) return report('INVALID', { issues: [{ code: 'EXPECTED_ONE_INPUT_PATH' }] });
  let fd;
  try {
    // A path is the normal sidecar interface; '-' permits bounded stdin reads.
    fd = args[0] === '-' ? 0 : openSync(args[0], 'r');
    // Read at most limit + 1, including streams/growing files. Never load an
    // unbounded file before checking its size and never execute collector text.
    ...
    if (length > MAX_BYTES) return report('INVALID', { issues: [{ code: 'JOURNAL_BYTE_LIMIT' }] });
    let raw;
    try { raw = new TextDecoder('utf-8', { fatal: true, ignoreBOM: true }).decode(bytes.subarray(0, length)); }
    catch { return report('INVALID', { issues: [{ code: 'INVALID_UTF8' }] }); }
    return analyzeJournal(raw);
  } catch {
    return report('INVALID', { issues: [{ code: 'INPUT_READ_FAILED' }] });
  } finally {
    if (fd !== undefined && args[0] !== '-') closeSync(fd);
  }
}
